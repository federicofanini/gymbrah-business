...
export function AthleteCodeBanner({ athleteCode }: { athleteCode: string }) {
    ...
    >
      ...
      <div className="relative z-50 flex flex-col space-y-4 w-[calc(100vw-16px)] max-w-[420px] border border-border p-4 bg-background rounded-lg shadow-lg">
        <div className="text-sm">
          Your athlete code is: <strong>{athleteCode}</strong>
        </div>
        <div className="flex justify-end space-x-2">
          <Button
            className="h-6"
            onClick={() => athleteCodeAction.execute({ athleteCode })}
          >
            Confirm
          </Button>
        </div>
